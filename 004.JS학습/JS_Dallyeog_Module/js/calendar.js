...
function MakeDallyeok(sel) { // sel - 달력 넣을 요소 선택자
        ...
        qsa(".date").forEach(
            (ele) =>
                (ele.onclick = () => {
                    ...
                    // 전달/다음달은 span 태그가 있으므로 구분한다!
                    let isSpan = ele.querySelector("span");
                    cg(isSpan);
                    // 없을 경우 null값이 나옴 -> if문에서 false 처리 됨
                    if (isSpan) {
                        // null이 아닐때만 true 처리되어 들어감
                        ...
                        if (cls) {
                            ...
                            // Number(문자형숫자) -> 숫자형 변환
                            // -,*,/ 연산은 브라우저가 자동변환해준다
                            // 그러나 + 연산은 문자 더하기 가능하므로 강제 형변환 해야 안전하다.
    ...
    // (2) 이전달력 출력하기 함수 //////////////
    this.prevCal = () => {
        // 이전월로 변경하여 this.initDallyeok()함수호출
    ...
    // (3) 다음달력 출력하기 함수 //////////////
    this.nextCal = () => {
        // 다음월로 변경하여 this.initDallyeok()함수호출
        // getMonth() 월가져오기 / setMonth() 월 셋팅하기!
        this.curr_date.setMonth(this.curr_date.getMonth() + 1);
        this.initDallyeok();
    }; ////////////// this.prevCal함수 //////////////
...
// default는 이름 변경없는 단 하나의 모듈을 내보낼 때 사용
