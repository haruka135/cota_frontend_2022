...
/* 
    [ 구현내용 ] : "돌아" 버튼을 클릭하면 멈춰있던 큐브가 돌아감 이 때, 버튼이 "멈춰!" 버튼으로 변경되어 있음
                
        - "멈춰!" 버튼을 클릭하면 돌고 있던 큐브가 멈춘다, 이 때 버튼이 "돌아" 버튼으로 변경되어 있음
*/
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. 대상선정
    // 1-1. 이벤트 대상 : .btngo
    const btngo = document.querySelector(".btngo"); // 단일한 요소를 담는다.
    ...
    btngo.onclick = () => { 
        ...
        // toggle() 메소드는 지정한 클래스가 있으면 빼고, 없으면 넣는다.

        // 2. 현재 큐브에 "on" 클래스가 있는지 여부 검사
